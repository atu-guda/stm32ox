...
{
#ifndef BMP_DEBUG
  i2c.recv_reg1( addr, reg_calibr_start, (uint8_t*)(&calibr), sizeof(calibr) );
...
int  BMP085::get_T_uncons( bool do_get )
{
  ...
  if( do_get ) {
    ...
    i2c.send_reg1( addr, reg_cmd, &cmd, 1 );
    delay_ms( t_wait_T );
    uint8_t tt[2];
    i2c.recv_reg1( addr, (int8_t)(reg_out), tt, sizeof(tt) );
...
int  BMP085::get_P_uncons( bool do_get )
{
  ...
  if( do_get ) {
    ...
    delay_ms( t_wait_P );
    uint8_t tp[3];
    i2c.recv_reg1( addr, (int8_t)(reg_out), tp, sizeof(tp) );
